...
public class Detection extends Activity {
	...
	/*
	 * Erkennt Dreiecke, Vierecke, Achtecke und Kreise und trennt diese, mit Hilfe der ausgewählten Farbe, vom Hintergrund.
	 * 
	...
	if(frameWert >= 1 && frameWert <= 3){
		...
		//Umwandlung des HSV-Bild in ein Binärbild mit den in der App gesetzten Farbwerten
		Core.inRange(mIntermediateMat, new Scalar(h_min, s_min, v_min),
				new Scalar(h_max, s_max, v_max), mIntermediateMat);
		...
		//Findet Countouren im Bild und trägt diese in eine Liste (contours) ein
		...
		for (int i = 0; i < contours.size(); i++)
		{
			...
			//Es werden nur Contouren, die eine Mindestgröße und eine bestimmte Anzahl an Ecken aufweisen für die weitere Berechnung berücksichtig
			...
			//Kreise einer gewissen Größe und mit mehr als 8 Ecken (da sonst Achtecke als Kreise erkannt werden)
		...
		//Die Farbinforamtionen des inputFrames werden in die ausgeschnittenen (weiße Pixel) Contouren des Binärbildes kopiert
	...
	public static Mat shapeDetection(CvCameraViewFrame inputFrame){
		...
		for (int i = 0; i < contours.size(); i++)
		{
			...
			Imgproc.approxPolyDP(contour2f, approx2f, Imgproc.arcLength(contour2f, true)*0.01 , true);
			approx.fromArray(approx2f.toArray());
